...
bool ImGui::TextLink(const char *label) {
  ...
  window->DrawList->AddLine(
      ...
      1.0f * (float)(int)
                 g.Style._MainScale); // FIXME-TEXT: Underline mode // FIXME-DPI
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags, float thickness) {
  ...
  if (flags & ImGuiSeparatorFlags_Vertical) {
    ...
    // calculating height for layout (e.g. bottom alignment of a status bar). In
                            ...
                            // already past the limits
        {
          ...
          float new_clicked_t =
              ScaleRatioFromValueT<TYPE, SIGNEDTYPE, FLOATTYPE>(
                  data_type, v_new, v_min, v_max, logarithmic_zero_epsilon,
                  zero_deadzone_halfsize);
    ...
    if (IsKeyPressed(ImGuiKey_LeftArrow)) {
      ...
      if (!is_new_line) {
        ...
        if (io.ConfigInputTextEnterKeepActive && !is_multiline) {
          // Queue reactivation, so that e.g. IsItemDeactivatedAfterEdit() will
          // work. (#9001)
  ...
  if (render_cursor || render_selection) {
    ...
    if (new_scroll_y != scroll_y) {
      const float scroll_max_y = ImMax(
          (text_size_y + style.FramePadding.y * 2.0f) - inner_size.y, 0.0f);
  ...
  if (render_cursor) {
    ...
    if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
      draw_window->DrawList->AddLine(
          ...
          1.0f * (float)(int)
                     style._MainScale); // FIXME-DPI: Cursor thickness (#7031)
  ...
  if ((flags & ImGuiColorEditFlags_NoBorder) == 0) {
    if (g.Style.FrameBorderSize > 0.0f)
      ...
      window->DrawList->AddRect(
          bb.Min, bb.Max, GetColorU32(ImGuiCol_FrameBg), rounding, 0,
          1.0f * (float)(int)
                     g.Style._MainScale); // Color buttons are often in need of
